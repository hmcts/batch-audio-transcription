...
3. The app is started locally (`cd frontend && pnpm dev`) and tested with Playwright in a separate terminal (`cd frontend && pnpm run test:e2e`, which defaults to `http://localhost:3000`) — the Playwright config has no `webServer`, so the app must already be running.
...
5. Once the PR is merged to `main`, the change is deployed to dev via the existing pipeline (push a `deploy-dev-*` tag from the app repo), tested using Playwright against dev (`cd frontend && PLAYWRIGHT_BASE_URL=<dev-url> pnpm run test:e2e`), and the affected App Service(s) are confirmed running (`GET /health` on the API, the frontend's `/batch` route).
